...
file = "support_uke_24.xlsx"  #navn til filen
data = pd.read_excel(file) # lese filen
#print(data)

# Hente ut data frå spesifikke kolonner

u_dag = data["Ukedag"].values              #lagrer data fra kolonne 1 (ukedag) en array
kl_slett = data["Klokkeslett"].values      #lagrer data fra kolonne 2 (Klokkeslett) en array
varighet = data["Varighet"].values         #lagrer data fra kolonne 3 (Samtales varighet) i en array
score = data["Tilfredshet"].values         #lagrer data fra kolonne 4 (Kundens tilfredshet) i en array      
...
antall_henvendelser_per_dag = data["Ukedag"].value_counts()                    # Antall henvendelser for hver ukedag
ukedager = ["Mandag", "Tirsdag", "Onsdag", "Torsdag", "Fredag"]                # Sørger for at ukedagene vises i en spesifikk rekkefølge
antall_henvendelser_per_dag = antall_henvendelser_per_dag[ukedager].fillna(0)  # Hvis noen ukedager mangler, kan vi sikre at alle er med, 
                                                                               # selv om det ikke er noen henvendelser
...
min_tid = np.min(varighet)                                                      # Minste samtaletid
max_tid = np.max(varighet)                                                      # Lengste samtaletid
...
antall_positive = sum((data["Tilfredshet"] >= 9) & (data["Tilfredshet"] <= 10))# Beregne antall positive kunder
antall_noytrale = sum((data["Tilfredshet"] >= 7) & (data["Tilfredshet"] <= 8)) # Beregne antall nøytrale kunder
antall_negative = sum((data["Tilfredshet"] >= 1) & (data["Tilfredshet"] <= 6)) # Beregne antall negative kunder

totale_tilbakemeldinger = antall_positive + antall_noytrale + antall_negative   # Totalt antall tilbakemeldinger
